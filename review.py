...
class appReview(object):
    ...
    def get_play_store(self, app_id, wait):
        ...
        with Chrome(str(self.webdriver_path), options=self.chrome_options) as driver:
            ...
            play_store_reviews['star'] = [int(re.findall('Rated (\d)', rate)[0]) for rate in play_store_reviews.star]
    ...
    def get_app_store(self, app_id, wait):
        ...
        with Chrome(str(self.webdriver_path), options=self.chrome_options) as driver:
            driver.get(f'https://apps.apple.com/hk/app/id{app_id}?l=en#see-all/reviews')
